...
function convert(body: string): { blocks: MEl[]; report: string[] } {
  const report: string[] = [];
  const { blocks } = convertDocx(docWith(body), report);
...
describe('convertDocx — lists', () => {
  it('groups consecutive bullet paragraphs into one ul', () => {
    const { blocks } = convert(li('uno', '1') + li('due', '1') + li('tre', '1'));
    expect(blocks).toHaveLength(1);
    expect(shape(blocks[0])).toBe('ul(li,li,li)');
    expect(textOf(blocks[0] as MEl)).toBe('unoduetre');
  });

  it('maps numeric formats to ol (letters and romans included)', () => {
    const { blocks, report } = convert(li('primo', '2') + li('secondo', '2'));
    expect(shape(blocks[0])).toBe('ol(li,li)');
    expect(report).toHaveLength(0);
  });

  it('nests deeper ilvl inside the previous item', () => {
    const { blocks } = convert(li('a', '2') + li('a1', '2', 1) + li('a2', '2', 1) + li('b', '2'));
    expect(blocks).toHaveLength(1);
    expect(shape(blocks[0])).toBe('ol(li[ol(li,li)],li)');
  });

  it('a plain paragraph ends the group; the next list starts fresh', () => {
    const { blocks } = convert(li('a', '1') + para('interruzione') + li('b', '1'));
    expect(blocks.map((b) => b.tag)).toEqual(['ul', 'p', 'ul']);
  });

  it('a numbering restart at the top level splits the lists', () => {
    const { blocks } = convert(li('a', '2') + li('b', '2') + li('x', '3'));
    expect(blocks.map((b) => b.tag)).toEqual(['ol', 'ol']);
    expect(shape(blocks[0])).toBe('ol(li,li)');
    expect(shape(blocks[1])).toBe('ol(li)');
  });

  it('kind change at the top level splits the lists', () => {
    const { blocks } = convert(li('a', '1') + li('n', '2'));
    expect(blocks.map((b) => b.tag)).toEqual(['ul', 'ol']);
  });

  it('numbering through the style chain makes a list item', () => {
    const { blocks } = convert(para('da stile', '<w:pStyle w:val="ListStyle"/>'));
    expect(shape(blocks[0])).toBe('ul(li)');
  });

  it('numId 0 cancels inherited numbering', () => {
  ...
  it('numFmt none is a plain paragraph', () => {
    const { blocks } = convert(li('nascosto', '5'));
    expect(blocks[0]?.tag).toBe('p');
  });

  it('lvlOverride wins over the abstract format', () => {
    const { blocks } = convert(li('override', '6'));
    expect(blocks[0]?.tag).toBe('ol');
  });

  it('missing definition falls back to ul with a report', () => {
    const { blocks, report } = convert(li('orfano', '99'));
    expect(blocks[0]?.tag).toBe('ul');
    expect(report.some((r) => r.includes('without a numbering definition'))).toBe(true);
  });

  it('exotic formats become a plain ol with a report', () => {
    const { blocks, report } = convert(li('esotico', '4'));
    expect(blocks[0]?.tag).toBe('ol');
    expect(report.some((r) => r.includes('chicago'))).toBe(true);
  });

  it('a numbered heading stays a heading, with a report', () => {
  ...
  it('a group starting deep re-nests under the later, shallower root', () => {
    const { blocks } = convert(li('profondo', '1', 1) + li('radice', '1'));
    expect(blocks).toHaveLength(1);
    expect(shape(blocks[0])).toBe('ul(li[ul(li)],li)');
  });

  it('list items drop translated indent (structure conveys it)', () => {
    const { blocks } = convert(li('testo', '1', 0, '<w:ind w:left="720"/>'));
    const item = (blocks[0]?.children ?? []).find((c): c is MEl => typeof c !== 'string');
    expect(item?.attrs['__wdf_style'] ?? '').not.toContain('margin-left');
  });

  it('nested lists serialize to a conforming document', () => {
    const { blocks } = convert(
