...
beforeAll(async () => {
  ...
  const bytes = await buildPackage(readDirFiles(join(examplesDir, 'delibera-pa')));
  wdfPath = join(work, 'delibera.wdf');
...
describe('WDF tools (T6.1)', () => {
  it('wdf_open verifies and summarizes the document', async () => {
    ...
    expect(out).toContain('Determinazione n. 87/2026');
    expect(out).toContain('Verification: VERIFIED');
    expect(out).toContain('sec-premesse');
  ...
  it('wdf_outline returns the ordered structure map', async () => {
    ...
    expect(outline.some((n) => n.id === 'tbl-impegni' && n.type === 'table')).toBe(true);
  });

  it('wdf_read with a section id returns the whole subtree', async () => {
    const state: McpState = {};
    await callTool(state, 'wdf_open', { path: wdfPath });
    const result = await callTool(state, 'wdf_read', { id: 'sec-dispositivo' });
    const out = text(result);
    expect(out).toContain('## Dispositivo {#h-dispositivo} {#sec-dispositivo}');
    expect(out).toContain('{#li-0001}');
    expect(out).toContain('| capitolo | esercizio | importo | esigibile |');
    expect(out).not.toContain('{#sec-premesse}');
  });
  ...
  it('wdf_read without id returns the full canonical markdown', async () => {
    ...
    expect(text(result)).toContain('# Determinazione n. 87/2026');
  });

  it('wdf_cite returns a resolvable, verified citation', async () => {
    const state: McpState = {};
    await callTool(state, 'wdf_open', { path: wdfPath });
    const result = await callTool(state, 'wdf_cite', { id: 'tbl-impegni' });
    const out = text(result);
    expect(out).toContain(
      'Citation: wdf:urn:uuid:7d444840-9dc0-5d1c-b745-1a56c4e5f6a7#tbl-impegni',
    );
    expect(out).toContain('Verified: yes');
    expect(out).toContain('Impegni di spesa per capitolo ed esercizio {#tbl-impegni}');
  });
