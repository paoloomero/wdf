...
| `wdf import <file-or-url>` | Convert a native Word `.docx`, HTML (Word / Google Docs exports, saved or live web pages) or Markdown into a valid `.wdf`. Options: `--standalone` (self-contained `.wdf.html`), `--with-source` (embed the original and the conversion report), `--with-pdf` (author-saved PDF rendition), `--id <urn:uuid:…>` / `--previous <file.wdf>` (revise an existing document: same id, ids of unchanged elements kept — spec §4.1, §6.4.4; URL imports derive the id from the address), `--embed-fonts`, `--full-page`, `--fetch-remote`. |
